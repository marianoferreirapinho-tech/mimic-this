...
socket.on("role:ready",({gameId,playerId})=>{const g=games.get(gameId);if(!g||g.status!=="role"||g.turn.actorId!==playerId)return;g.status="record";emitGame(g)});
socket.on("turn:recorded",({gameId,playerId,videoLabel})=>{const g=games.get(gameId);if(!g||g.turn.actorId!==playerId)return;g.turn.videos.push(videoLabel||`Video ${g.turn.videos.length+1}`);g.status="watch";emitGame(g)});
